...
class PipelineSingleton {
  ...
  static nerInstance = null;

  static embedTask = 'feature-extraction';
  static embedModel = 'Xenova/bge-small-en-v1.5';
  static embedInstance = null;

  static async getNER() {
    if (this.nerInstance === null) {
      this.nerInstance = pipeline(this.nerTask, this.nerModel, { quantized: true });
    }
  ...
  static async getEmbedding() {
    if (this.embedInstance === null) {
      this.embedInstance = pipeline(this.embedTask, this.embedModel, { quantized: true });
    }
